  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: 24, fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 600, color: '#1C2B28', margin: 0 }}>Calcoli GHG</h1>
          <p style={{ fontSize: 13, color: '#6b7280', margin: '4px 0 0' }}>Carica bollette o inserisci dati manualmente</p>
        </div>
      ...
      {/* Period list */}
      {periods.length === 0 ? (
        <div style={{ padding: '24px 20px', textAlign: 'center', color: '#999', background: '#F4F8F7', borderRadius: 10, fontSize: 13 }}>
          Nessun periodo ancora. Carica una bolletta sopra per iniziare, oppure{' '}
          <a href={`/clients/${clientId}/periods/new`} style={{ color: '#2563eb' }}>inserisci manualmente</a>.
        </div>
      ) : (
        <>
          <div style={{ fontSize: 13, fontWeight: 500, color: '#999', marginBottom: 8 }}>Periodi inseriti</div>
          {periods.map(p => {
              ...
              >
                ...
                {s && (
                  <div style={{ display: 'flex', gap: 16, fontSize: 12 }}>
                    <span style={{ color: '#15803d', fontWeight: 500 }}>S1: {fmtCo2(s.scope1_co2e_kg)}</span>
                    <span style={{ color: '#1d4ed8', fontWeight: 500 }}>S2: {fmtCo2(s.scope2_lb_co2e_kg)}</span>
                    <span style={{ color: '#1C2B28', fontWeight: 700, minWidth: 60, textAlign: 'right' }}>{fmtCo2(s.total_co2e_kg)}</span>
                  </div>
                )}
                <span style={{
